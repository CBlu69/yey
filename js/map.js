...
export function initMap(user) {
    ...
    function createMap() {
        ...
        if (!mapContainer) { setTimeout(createMap, 300); return }
        ...
        new MutationObserver(() => { if (mapContainer.offsetParent !== null) map.invalidateSize() }).observe(mapContainer, { attributes: true, attributeFilter: ['class'] })
    }
    ...
    function updateUserMarker() {
        ...
        if (userMarker) userMarker.setLatLng(userLocation)
            ...
            userMarker = L.marker(userLocation, {
                icon: L.divIcon({ html: `<div style="font-size:24px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.3))">📍</div>`, className: 'user-marker', iconSize: [24, 24] })
    ...
    async function startSharing() {
        if (!userLocation) {
            try {
                const pos = await new Promise((r, rej) => navigator.geolocation.getCurrentPosition(r, rej, { enableHighAccuracy: true, timeout: 10000 }))
        ...
        await supabase.from('active_sharings').upsert({
            user_id: String(cu.id), user_name: cu.name, user_avatar: cu.avatar || '👤',
            is_active: true, latitude: userLocation.lat, longitude: userLocation.lng,
                ...
                async (pos) => {
                    ...
                    if (sharingActive) {
                        await supabase.from('active_sharings').update({
                            latitude: userLocation.lat, longitude: userLocation.lng, updated_at: new Date().toISOString()
        ...
        if (!sharingMarker) {
            ...
            sharingMarker = L.marker(userLocation, {
                icon: L.divIcon({ html: `<div style="width:20px;height:20px;background:#2ed573;border:3px solid #fff;border-radius:50%;box-shadow:0 0 15px rgba(46,213,115,0.8);animation:pulse 1.5s infinite;"></div>`, className: 'sharing-marker', iconSize: [20, 20] })
    ...
    async function stopSharing() {
        const cu = getCurrentUser()
        sharingActive = false
        if (cu) await supabase.from('active_sharings').update({ is_active: false, updated_at: new Date().toISOString() }).eq('user_id', String(cu.id))
    ...
    async function checkActiveSharing() {
        const cu = getCurrentUser()
        if (!cu) return
        const { data } = await supabase.from('active_sharings').select('*').eq('user_id', String(cu.id)).eq('is_active', true).single()
        if (!data) return
        sharingActive = true
        sharingStartTime = new Date(data.started_at).getTime()
        if (data.latitude && data.longitude) { userLocation = { lat: data.latitude, lng: data.longitude }; updateUserMarker() }
        if (navigator.geolocation) {
            sharingWatchId = navigator.geolocation.watchPosition(
                async (pos) => {
                    userLocation = { lat: pos.coords.latitude, lng: pos.coords.longitude }
                    updateUserMarker()
                    if (sharingActive) await supabase.from('active_sharings').update({ latitude: userLocation.lat, longitude: userLocation.lng, updated_at: new Date().toISOString() }).eq('user_id', String(cu.id))
                    if (sharingMarker) sharingMarker.setLatLng(userLocation)
                },
                () => { },
        ...
        updateSharingTimer()
        sharingTimer = setInterval(updateSharingTimer, 1000)
        window.showToast('📍 اشتراک از قبل فعال بود', 'info')
    }
    ...
    async function loadAllMapData() {
        ...
        const { data: sharings } = await supabase.from('active_sharings').select('*').eq('is_active', true)
        if (sharings) {
    ...
    function addFriendMarker(share) {
        ...
        const marker = L.marker([share.latitude, share.longitude], {
            icon: L.divIcon({
                html: `<div style="position:relative;width:36px;height:36px;border-radius:50%;background:rgba(46,213,115,0.3);display:flex;align-items:center;justify-content:center;animation:pulse 2s infinite;">${avImg}<div style="position:absolute;bottom:-3px;left:50%;transform:translateX(-50%);width:10px;height:10px;background:#2ed573;border-radius:50%;border:2px solid #fff;"></div></div>`,
    ...
    document.getElementById('add-pin-btn')?.addEventListener('click', async () => {
        ...
        map.once('click', async (e) => {
            ...
            const cu = getCurrentUser()
            const { data, error } = await supabase.from('pins').insert([{ name: pinName, latitude: lat, longitude: lng, user_id: cu?.id, user_name: cu?.name }]).select()
    ...
    function addPinToMap(pin) {
        const cu = getCurrentUser()
        const isOwner = pin.user_id === cu?.id
        const marker = L.marker([pin.latitude, pin.longitude], {
            icon: L.divIcon({ html: `<div style="font-size:28px;filter:drop-shadow(0 2px 4px rgba(0,0,0,0.3))">📌</div>`, className: 'pin-marker', iconSize: [28, 28] })
        }).addTo(map)
    ...
    window.deletePin = async (pinId) => {
        ...
        const p = allPins.find(x => x.id === pinId)
                ...
                btn.onclick = function (e) {
                    ...
                    if (userLocation) {
                        ...
                        navigator.geolocation.getCurrentPosition(
                            pos => { userLocation = { lat: pos.coords.latitude, lng: pos.coords.longitude }; updateUserMarker(); map.setView([userLocation.lat, userLocation.lng], 16, { animate: true, duration: 1 }) },
    ...
    supabase.channel('map-updates')
        ...
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'active_sharings' }, (p) => {
            const share = p.new
            const cu = getCurrentUser()
            if (share.user_id === String(cu?.id)) return
            const old = allPins.find(x => x.id === `share-${share.user_id}`)
            if (old?.marker) map.removeLayer(old.marker)
            allPins = allPins.filter(x => x.id !== `share-${share.user_id}`)
            if (share.is_active && share.latitude && share.longitude) addFriendMarker(share)
        })
        .subscribe()
